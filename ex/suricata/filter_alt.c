/*
 * ex/sur_filter.c with the DADDR branch returning 7 instead of 0.  Its only
 * job is to be different there: the checker separating the two is what shows
 * the second lookup is reached, which it was not while the packet region was
 * 32 bytes and the daddr read at offset 30..33 overran it.
 *
 * Derived from Suricata ebpf/filter.c, verbatim from #define DEBUG onwards; only the
 * includes are replaced, by ex/sur_common.h, so it builds without a kernel
 * tree.  A real production filter, and the one that exercises the most of the
 * translator at once: two map lookups, a store through a returned value
 * pointer, LD_ABS and LD_IND packet reads, and a ctx store (skb->cb[0]).
 *
